...
''',
    ...
    // `;` must be percent-encoded — Tesla's gateway treats raw `;` as a
    // query-param separator and silently drops everything after the
    // first endpoint. Location is optional because requesting location_data
    // displays Tesla's location-sharing indicator in the vehicle.
...
if 'type TeslaLocationAwareAdapter' not in text:
    replace_once(
        middleware,
...
''',
        '''  /** Fetch vehicle data and update the cache. Routine telemetry deliberately
...
''',
    ...
    it("fetches location once on plug-in, then reuses it", async () => {
      adapter.state = buildVehicleChargeState({
        isPluggedIn: true,
        latitude: 43.6,
        longitude: 1.4,
    ...
    it("allows a manual force refresh to request location", async () => {
      adapter.state = buildVehicleChargeState({
        isPluggedIn: false,
        latitude: 43.6,
        longitude: 1.4,
