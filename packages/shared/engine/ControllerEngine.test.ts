...
describe("ControllerEngine", () => {
  ...
  describe("mode dispatch", () => {
    ...
    it("stops an active Solar Only charge after a safety trip", () => {
      const engine = new ControllerEngine();
      const output = engine.decide(makeInput({
        configOverrides: {
      ...
      const decision = output.decisions.get("V1");
      expect(decision?.action).toBe("stop");
      expect(decision?.reason).toBe("safety_trip");
      expect(decision?.detail).toContain(
        "automatic solar charging interrupted",
      );
    });

    it("keeps the safety stop authoritative during a partially persisted reset", () => {
      const engine = new ControllerEngine();
      const output = engine.decide(makeInput({
        configOverrides: {
        ...
        vehicle: { mode: "auto", state: { isCharging: true } },
      }));

      expect(output.decisions.get("V1")?.action).toBe("stop");
      expect(output.decisions.get("V1")?.reason).toBe("safety_trip");
    });

    it("does not override manual Charge Now during a safety trip", () => {
      const engine = new ControllerEngine();
      const output = engine.decide(makeInput({
        configOverrides: {
          chargingEnabled: false,
        ...
        vehicle: { mode: "charge_now", state: { isCharging: true } },
      }));
      ...
      expect(decision?.reason).toBe("safety_trip");
      expect(decision?.detail).toContain(
        "manual Charge Now remains in control",
      );
    });

    it("distinguishes a voluntary automation pause from a safety stop", () => {
      const engine = new ControllerEngine();
      const output = engine.decide(makeInput({
        configOverrides: {
          chargingEnabled: false,
          chargingDisabledReason: "user",
        },
        vehicle: { mode: "vacation", state: { isCharging: true } },
      }));

      const decision = output.decisions.get("V1");
      expect(decision?.action).toBe("none");
      expect(decision?.reason).toBe("charging_disabled");
      expect(decision?.detail).toBe("Automatic charging paused by user");
    });
