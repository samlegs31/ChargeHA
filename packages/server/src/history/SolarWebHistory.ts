...
function deduplicateRows(rows: readonly VehicleChargeHistoryRowInput[]): VehicleChargeHistoryRowInput[] {
  const byExternalId = new Map<string, VehicleChargeHistoryRowInput>();
  for (const row of rows) byExternalId.set(row.externalId, row);
  return [...byExternalId.values()];
}

async function fetchRangeInDailyChunks(
  token: string,
  pvSystemId: string,
  fromIso: string,
  toIso: string,
  ...
  if (!Number.isFinite(start) || !Number.isFinite(end) || end < start) {
    throw new SolarWebHistoryError("Invalid Solar.web history date range");
  }
  ...
  while (cursor < end) {
    const chunkEnd = Math.min(cursor + MAX_RANGE_MS, end);
    const chunk = await fetchPages(
      token,
      pvSystemId,
      new Date(cursor).toISOString().replace(".000Z", "Z"),
      new Date(chunkEnd).toISOString().replace(".000Z", "Z"),
      fetchFn,
    );
    samplesRead += chunk.samplesRead;
    rows.push(...chunk.rows);
    cursor = chunkEnd;
  }

  return { samplesRead, rows: deduplicateRows(rows) };
}

function sumWh(rows: readonly VehicleChargeHistoryRowInput[], pick: (row: VehicleChargeHistoryRowInput) => number): number {
...
export async function fetchSolarWebHomeEvHistory(input: SolarWebHistoryImportInput, fetchFn: FetchFn = fetch): Promise<SolarWebHistoryResult> {
  ...
  const history = await fetchRangeInDailyChunks(
    token,
    input.pvSystemId,
